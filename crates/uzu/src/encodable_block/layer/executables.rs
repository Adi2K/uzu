...
use crate::{
    ...
    config::{DecoderLayerConfig, DecoderLayerType, MixerConfig, NormalizationConfig},
    encodable_block::{
        Attention, DeltaNetMixer, EncodingParameters, Linear, MambaMixer, Mlp, QKNorm, RMSNorm, RMSNormHadamard, Rope,
        ShortConvMixer,
    },
...
impl<B: Backend> LayerExecutables<B> {
    ...
    pub fn new(
        ...
        let pre_attention_residual_add = layer_index > 0;
        ...
        let (pre_attention_norm, mixer) = match &layer_config.mixer_config {
            MixerConfig::Attention(attention_config) => {
                ...
                let pre_attention_norm = make_norm_block(
                    context,
                    intermediate_data_type,
        ...
        let pre_mlp_norm = make_norm_block(
            context,
            intermediate_data_type,
